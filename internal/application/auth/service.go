...
func (s Service) Refresh(ctx context.Context, rawRefreshToken string) (TokenPair, error) {
	...
	if err != nil {
		return TokenPair{}, shared.New(shared.CodeUnauthenticated, "refresh token is invalid")
	}
	if !session.Active(s.Clock.Now()) {
		if session.UsedAt != nil {
			_ = s.Sessions.RevokeFamily(ctx, session.FamilyID, s.Clock.Now())
		}
		return TokenPair{}, shared.New(shared.CodeUnauthenticated, "refresh token is expired, revoked, or already used")
	}
	...
	err = s.Transactions.WithinTransaction(ctx, func(txCtx context.Context) error {
		...
		replacement, err := domain.NewRefreshSession(replacementID, account.OrganizationID, account.EmployeeID, session.FamilyID, rawReplacement, s.Clock.Now(), s.RefreshTTL)
		if err != nil {
			return err
		}
		expectedVersion := session.Version
		if err := session.Consume(replacement.ID, expectedVersion, s.Clock.Now()); err != nil {
			return err
		}
		if err := s.Sessions.Update(txCtx, session, expectedVersion); err != nil {
			return err
		}
		if err := s.Sessions.Insert(txCtx, *replacement); err != nil {
			return err
		}
		...
		pair = TokenPair{AccessToken: access, RefreshToken: rawReplacement, AccessExpiresAt: accessExpiry, RefreshExpiresAt: replacement.ExpiresAt}
		return nil
	})
	if err != nil {
		return TokenPair{}, err
