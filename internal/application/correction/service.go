...
func (s Service) Approve(ctx context.Context, command ApproveCommand) (RequestResult, error) {
	...
	if !command.Actor.CanAccessProject(command.OrganizationID, original.ProjectID) {
		return RequestResult{}, shared.New(shared.CodeForbidden, "correction is outside reviewer project scope")
	}
	review, err := timesheet.BeginCorrectionReview(original, replacement, command.Correction, command.OrganizationID, command.Actor.EmployeeID, s.Clock.Now())
	if err != nil {
		return RequestResult{}, err
	}
	before := original
	entryVersion := original.Version
	correctionVersion := command.Correction.Version
	if err := review.Apply(); err != nil {
		return RequestResult{}, err
	}
	err = s.Transactions.WithinTransaction(ctx, func(txCtx context.Context) error {
		if err := s.Entries.Update(txCtx, review.Original, entryVersion); err != nil {
			return err
		}
		if err := s.Corrections.Update(txCtx, review.Correction, correctionVersion); err != nil {
			return err
		}
		eventID, err := s.IDs.New("audit")
		if err != nil {
		...
		event, err := audit.NewEvent(eventID, command.OrganizationID, command.Actor.EmployeeID, "api", "time_entry", review.Original.ID, "corrected", review.Correction.Reason, command.RequestID, before, review.Original, review.ReviewedAt)
		if err != nil {
			return err
		}
		return s.Audits.Append(txCtx, event)
	...
	return RequestResult{Correction: review.Correction, Replacement: review.Replacement}, nil
}
